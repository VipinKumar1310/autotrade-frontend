...
import { ChevronDown, AlertTriangle, Trash2, Pencil } from 'lucide-react';
...
const tagConfig: Record<MessageTag, { 
  ...
  PARSED: { 
    label: 'PARSED', 
    className: 'bg-dark-muted/20 text-dark-muted',
    priority: 2
  },
  TRADE_EXECUTED: { 
    label: 'EXECUTED', 
...
export function MessageBubble({ message, onSignalClick }: MessageBubbleProps) {
  ...
  return (
    <div className="w-full">
      <div 
        className={clsx(
          'relative bg-dark-card border border-dark-border rounded-lg p-3 max-w-[85%]',
          hasSignal && 'cursor-pointer hover:border-dark-muted',
      ...
      >
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          {hasSignal && (
            <button 
              className="flex items-center gap-0.5 text-[10px] text-dark-muted hover:text-white transition-colors"
              onClick={(e) => {
                e.stopPropagation();
                onSignalClick?.();
              }}

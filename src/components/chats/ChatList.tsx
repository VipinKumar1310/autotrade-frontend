...
export function ChatList() {
  ...
  const { telegramProviders, telegramMessages } = useStore();
      ...
      {connectedProviders.map((provider) => {
        ...
        return (
          <div
            ...
            className="flex items-center gap-3 p-4 cursor-pointer transition-colors hover:bg-dark-card/50 active:bg-dark-card"
          >
            {/* Avatar */}
            <div 
            ...
            {/* Arrow */}
            <ChevronRight size={20} className="text-dark-muted flex-shrink-0" />
          </div>
        );
      })}
    </div>

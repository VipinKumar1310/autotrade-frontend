...
const getDateRange = (period: TimePeriod, customRange?: { start: string; end: string }): { start: Date; end: Date } => {
  const now = new Date();
...
export default function DashboardPage() {
  ...
  const [signalsTab, setSignalsTab] = useState<'all' | 'executed' | 'skipped' | 'pending'>('all');
  ...
  const filteredData = useMemo(() => {
    ...
    let filteredTrades = trades.filter(trade => {
      ...
      const inDateRange = tradeDate >= dateRange.start && tradeDate <= dateRange.end;
      
      const matchesAutomation = selectedAutomations.length === 0 || 
      ...
      const matchesChannel = selectedChannels.length === 0 || 
        (automation && selectedChannels.includes(automation.telegram_provider_id));
  ...
  }, [trades, parsedSignals, automations, timePeriod, customDateRange, selectedAutomations, selectedChannels]);

  // Calculate analytics
  const analytics = useMemo(() => {
    const closedTrades = filteredData.trades.filter(t => t.status === 'closed');
    ...
    const executedSignals = filteredData.signals.filter(s => s.execution_status === 'executed').length;
    const skippedSignals = filteredData.signals.filter(s => s.execution_status === 'skipped').length;
  ...
  const displayTrades = useMemo(() => {
    ...
    return tradesForDisplay.sort((a, b) => 
      new Date(b.entry_time).getTime() - new Date(a.entry_time).getTime()
    );
  }, [filteredData.trades, tradesTab]);

  const getAutomationById = (id: string) => automations.find((a) => a.id === id);
  const getSignalById = (id: string | null) => id ? parsedSignals.find((s) => s.id === id) : null;
  const getProviderById = (id: string) => telegramProviders.find((p) => p.id === id);
  ...
  const displaySignals = useMemo(() => {
    ...
    let filtered = parsedSignals.filter(signal => {
      const signalDate = new Date(signal.parsed_at);
      if (signalDate < dateRange.start || signalDate > dateRange.end) return false;
      
      if (selectedAutomations.length > 0 && !selectedAutomations.includes(signal.automation_id)) {
        return false;
      }
      
      if (selectedChannels.length > 0 && !selectedChannels.includes(signal.provider_id)) {
        return false;
      }
    ...
    if (signalsTab === 'executed') {
      ...
      filtered = filtered.filter(s => s.execution_status === 'pending' || s.execution_status === 'pending_manual');
    }
    
    return filtered.sort((a, b) => new Date(b.parsed_at).getTime() - new Date(a.parsed_at).getTime());
  }, [parsedSignals, timePeriod, customDateRange, selectedAutomations, selectedChannels, signalsTab]);

  // Get signal status display config
  const getSignalStatusConfig = (status: string) => {
    switch (status) {
      case 'executed':
        return { label: 'Executed', icon: Check, className: 'text-profit bg-profit/10' };
      case 'skipped':
        return { label: 'Skipped', icon: Ban, className: 'text-loss bg-loss/10' };
      case 'pending_manual':
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className={clsx("p-3 border", theme === 'dark' ? 'bg-dark-card border-dark-border' : 'bg-white border-gray-200 shadow-sm')}>
            <div className="flex items-center gap-2 mb-1">
              <div className={clsx(
                ...
                analytics.totalPnL >= 0 ? 'bg-profit/20' : 'bg-loss/20'
              )}>
          ...
          <div className={clsx("p-3 border", theme === 'dark' ? 'bg-dark-card border-dark-border' : 'bg-white border-gray-200 shadow-sm')}>
            ...
            {analytics.openTrades > 0 && (
              <p className="text-[10px] text-profit">
        ...
        <div className={clsx("sticky top-0 z-30 border-b", theme === 'dark' ? 'bg-dark-bg border-dark-border' : 'bg-gray-50 border-gray-200')}>
          ...
          <div className="flex items-center gap-1 px-4 pb-2">
            {mainTab === 'trades' ? (
              ...
              <>
                {(['all', 'executed', 'skipped', 'pending'] as const).map((tab) => (
                  <button
                    key={tab}
          ...
          {mainTab === 'trades' ? (
            ...
            displayTrades.length === 0 ? (
              ...
              displayTrades.map((trade) => {
                ...
                const currentPnl = isOpen && trade.current_price 
                  ? (trade.current_price - trade.entry_price) * trade.quantity * (isBuy ? 1 : -1)
                  : trade.pnl;
                const currentPnlPercent = isOpen && trade.current_price 
            ...
            displaySignals.length === 0 ? (
              ...
              displaySignals.map((signal) => {
                ...
                const statusConfig = getSignalStatusConfig(signal.execution_status);
                const StatusIcon = statusConfig.icon;
                const linkedTrade = filteredData.trades.find(t => t.signal_id === signal.id);
                        ...
                        <div className={clsx("px-3 pb-3 pt-2 border-t", theme === 'dark' ? 'border-dark-border' : 'border-gray-200')}>
                          ...
                          {signal.execution_status === 'skipped' && signal.skip_reason && (
                            <div className="mt-2 p-2 bg-loss/10 border border-loss/20 text-[10px]">
                              <span className="text-loss font-medium">Skip Reason: </span>
                              <span className="text-loss/80">{signal.skip_reason}</span>
                            </div>
                          )}
                          ...
                          {signal.execution_status === 'pending_manual' && (
                            <div className="mt-2 p-2 bg-warning/10 border border-warning/20 text-[10px]">
                              <span className="text-warning font-medium">Awaiting manual confirmation</span>
                            </div>
                          )}
                          ...
                          {linkedTrade && (
                            <div className="mt-2 p-2 bg-profit/10 border border-profit/20 text-[10px]">
                              <span className="text-profit font-medium">Trade P&L: </span>
                              <span className={clsx('font-semibold', linkedTrade.pnl >= 0 ? 'text-profit' : 'text-loss')}>
                                {linkedTrade.pnl >= 0 ? '+' : ''}₹{linkedTrade.pnl.toLocaleString()}
                              </span>
                            </div>
                          )}
